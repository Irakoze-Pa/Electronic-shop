...
interface Props {
  noun: "brand" | "category";
  load: () => Promise<CatalogEntity[]>;
  create: (input: EntityInput) => Promise<unknown>;
  update: (id: string, input: EntityInput) => Promise<unknown>;
...
export function CatalogEntityManager({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const input: EntityInput = {
      ...
      media,
      status: String(data.get("status")) as CatalogStatus,
    };
    setBusy(true);
    try {
      if (editing) await update(editing._id, input);
        ...
        >
          ...
          <div className="sm:col-span-2">
            <input
              ...
              placeholder={`${noun === "brand" ? "Logo" : "Image"} URL (optional)`}
              type="url"
              value={media}
            />
            <div className="mt-3">
              <ImageUploadButton onUploaded={([url]) => setMedia(url ?? "")} />
            </div>
          </div>
          <textarea
