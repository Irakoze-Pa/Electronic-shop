...
export function ProductDetailsPage() {
  ...
  function requireLogin() {
    ...
    navigate("/login", { state: { from: `${location.pathname}${location.search}`, backgroundLocation: location } });
    return true;
  }
  ...
  return <main className="bg-white px-5 py-10 lg:px-8"><div className="mx-auto max-w-7xl"><nav className="text-sm text-slate-500"><Link to="/">Home</Link> / <Link to="/shop">Shop</Link> / {product.name}</nav><div className="mt-8 grid gap-12 lg:grid-cols-2"><section><div className="aspect-square overflow-hidden rounded-3xl bg-slate-100">{product.images[selectedImage] ? <img alt={product.name} className="h-full w-full object-cover" src={product.images[selectedImage]} /> : <div className="grid h-full place-items-center text-7xl font-black text-[#0ea5e9]/20">BBG</div>}</div><div className="mt-4 flex gap-3">{product.images.map((url, index) => <button aria-label={`View image ${index + 1}`} className={`size-20 overflow-hidden rounded-xl border-2 ${index === selectedImage ? "border-[#0ea5e9]" : "border-transparent"}`} key={url} onClick={() => setSelectedImage(index)} type="button"><img alt="" className="h-full w-full object-cover" src={url} /></button>)}</div></section><section className="self-center"><p className="text-sm font-bold uppercase tracking-wider text-[#f97316]">{product.brand.name}</p><h1 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">{product.name}</h1><p className="mt-3 text-sm text-slate-500">Code: {product.code} · {product.category.name}</p><div className="mt-7 flex flex-wrap items-center gap-3"><span className="text-3xl font-black text-[#0ea5e9]">{currencyFormatter.format(product.price)}</span>{product.oldPrice && <span className="text-lg text-slate-400 line-through">{currencyFormatter.format(product.oldPrice)}</span>}{discount > 0 && <span className="rounded-full bg-[#f97316]/10 px-3 py-1 text-sm font-bold text-[#f97316]">Save {discount}%</span>}</div><p className={`mt-5 text-sm font-bold ${product.stock ? "text-emerald-600" : "text-red-600"}`}>{product.stock ? `${product.stock} ${product.unit} available` : "Out of stock"}</p><p className="mt-6 leading-7 text-slate-600">{product.description || product.shortDescription}</p><div className="mt-8 flex flex-wrap gap-3"><div className="flex items-center rounded-xl border border-slate-300"><button aria-label="Decrease quantity" className="size-12 text-xl disabled:text-slate-300" disabled={quantity <= 1 || !product.stock} onClick={() => setQuantity((value) => Math.max(1, value - 1))} type="button">−</button><span className="w-10 text-center font-bold">{quantity}</span><button aria-label="Increase quantity" className="size-12 text-xl disabled:text-slate-300" disabled={!product.stock || quantity >= product.stock} onClick={() => setQuantity((value) => Math.min(product.stock || 1, value + 1))} type="button">+</button></div><button className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#0ea5e9] px-6 py-3 font-bold text-white disabled:bg-slate-300" disabled={!product.stock || cart.busyProductId === product._id} onClick={() => void addToCart()} type="button"><CartIcon className="size-5" /> {cart.busyProductId === product._id ? "Adding…" : "Add to cart"}</button><button aria-label={saved ? "Remove from wishlist" : "Add to wishlist"} className={`grid size-12 place-items-center rounded-xl border border-slate-300 ${saved ? "text-[#f97316]" : ""}`} disabled={wishlist.busyProductId === product._id} onClick={() => void toggleWishlist()} type="button"><HeartIcon className={`size-5 ${saved ? "fill-current" : ""}`} /></button></div>{actionMessage && <p aria-live="polite" className="mt-3 text-sm font-semibold text-slate-600">{actionMessage}</p>}<div className="mt-9 border-t border-slate-200 pt-7"><h2 className="font-black">Specifications</h2>{Object.keys(product.specifications).length ? <dl className="mt-4 divide-y divide-slate-100">{Object.entries(product.specifications).map(([label, value]) => <div className="grid grid-cols-2 gap-4 py-3 text-sm" key={label}><dt className="text-slate-500">{label}</dt><dd className="font-semibold">{value}</dd></div>)}</dl> : <p className="mt-3 text-sm text-slate-500">No specifications provided.</p>}</div></section></div><section className="mt-20 border-t border-slate-200 pt-14"><h2 className="text-3xl font-black">Related products</h2><div className="mt-8">{related.length ? <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{related.map((item) => <ProductCard key={item._id} product={item} />)}</div> : <EmptyState message="No related products yet." />}</div></section></div></main>;
}
