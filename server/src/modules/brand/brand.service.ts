...
export const brandService = {
  ...
  async update(id: string, input: Partial<BrandInput>) { const slug = input.name ? slugify(input.name) : undefined; if (slug && (await Brand.exists({ slug, _id: { $ne: id } }))) throw new AppError("Brand slug already exists", 409); const brand = await Brand.findByIdAndUpdate(id, { ...input, ...(slug ? { slug } : {}) }, { new: true, runValidators: true }); if (!brand) throw new AppError("Brand not found", 404); return brand; },
