...
export const productService = {
  async list(query: ProductQuery, includeInactive = false) {
    ...
    const [items, total] = await Promise.all([
      Product.find(filter)
        ...
        .populate("brand", "name slug status")
  ...
  async get(id: string, includeInactive = false) {
  ...
  async create(input: ProductInput) {
    ...
    if (await Product.exists({ $or: [{ slug }, { code }] }))
      throw new AppError("Product slug or code already exists", 409);
  ...
  async update(id: string, input: Partial<ProductInput>) {
    ...
    const nextOldPrice =
      input.oldPrice === undefined ? existing.oldPrice : input.oldPrice;
    ...
    if (
      ...
      (await Product.exists({
        _id: { $ne: id },
        $or: [...(slug ? [{ slug }] : []), ...(code ? [{ code }] : [])],
      }))
    )
      throw new AppError("Product slug or code already exists", 409);
    const product = await Product.findByIdAndUpdate(
      id,
      { ...input, ...(slug ? { slug } : {}), ...(code ? { code } : {}) },
